...
int GetSBuvJS::AddImg(const cv::Mat& Input)
{
...
//        bimg+= maskh;
    ...
    for(auto &it:lines)
    {
        ...
        //_log << "k: " << k << endl;
        if( k > 0.0968 && k < 3) slines.push_back(it);
        //else if(k > 0.08) linesr.push_back(it);
    }

    lines.clear();
    int leftEndPoint= _imgCols/2;
    int rightEndPoint= _imgCols/2;
    for(auto &line:slines)
    {
        for(int i= 0;i < 4;i++) _log << line[i] << " ";
        _log << endl;
        if(leftEndPoint > line[0]) leftEndPoint= line[0];
    }
    for(auto &line:slines)
    {
        if(line[0]-leftEndPoint < 80) lines.push_back(line);
    }
    for(auto &it:lines)
    {
        if(rightEndPoint < it[2]) rightEndPoint= it[2];
    }

    red(cv::Rect(rightEndPoint,0,red.cols-(rightEndPoint),red.rows))= 0;

    cv::Point2d slcut0,slcut1;
    vector<double> kv,cv;
...
//        }
    ...
    else
    {
        ...
        cv::threshold(input(roisl)(roils),itemp,th,255,8);
        //cv::adaptiveThreshold(input(roisl)(roils),itemp,255,0,0,11,-2);
        if(_save == 2) cv::imwrite(_logpd+to_string(_imgNum)+"ir.jpg",input(roisl)(roils));
...
bool GetSBuvJS::GetHoughLine(const cv::Mat &bimg,cv::Point2d &slcut0,cv::Point2d &slcut1,int SIGN,double ratio0, double k0,double k1)
{
    //vector<cv::Vec4i> selectedLinesL,selectedLinesR,selectedLines;
