...
//    return 0;
    ...
    {
        SIGN= 1;
        roisl.x= _icenter.x-_roiWidthShift/2;
...
//        cv::Rect roilPts= cv::Rect(btlx[(btlx.size()-1)/2],0,itemp.cols-btlx[(btlx.size()-1)/2],itemp.rows);
        ...
        roisl.width= uvr.x;
        cout << "roisl2: " << roisl << endl;
        input(roisl)(cv::Rect(0,0,uvl.x,roisl.height))= 2;
        if(GetHoughLine(input(roisl),slcut0,slcut1,SIGN,0.8)) {
        //    rcut0.y= (rcut0.x-sline[2])*(sline[1]-sline[3])/(sline[0]-sline[2])+sline[3];
        //    rcut1.y= (rcut1.x-sline[2])*(sline[1]-sline[3])/(sline[0]-sline[2])+sline[3];
        ...
        if(_save > 0)
        {
            cv::circle(Input,_uv[_imgNum],15,cv::Scalar(255,255,0),2);
            cv::circle(Input,_uvl[_imgNum],15,cv::Scalar(255,0,0),2);
...
//    vector<int> deltal4,lps,rps;
//    int lp,rp,countp= 0;
//    for(auto &it:lines) deltal4.push_back(it[2]);
//    //rp= *(deltal4.begin());
//    for(auto &d:deltal4) {
//        if(countp == 0) rps.push_back(d);
//        else {
//            sort(rps.begin(),rps.end());
//            for(auto &r:rps) cout << r << " ";
//            cout << " rend\n";
//            rp= rps[(rps.size()-1)/2];
//            if(abs(rp-d) < 19) rps.push_back(d);
//            else lps.push_back(d);
//        }
//        countp++;
//    }
//    sort(rps.begin(),rps.end());
...
//    }
    ...
    int lp,rp,countp= 0;
    for(auto &it:lines) deltal4.push_back(it[2]);
    sort(deltal4.begin(),deltal4.end());
    rp= *(deltal4.end()-1);
...
//        else if(k > k0 && k < k1 && it[0] > _uvl[_imgNum].x-(_icenter.x-_roiWidthShift)-77) slines.push_back(it);
        if(k > k0 && k < k1 && it[2] > rp-69 && it[0] > 88) slines.push_back(it);
        //else if(k > 0.08) linesr.push_back(it);
    }
