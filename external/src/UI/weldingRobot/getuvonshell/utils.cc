...
bool ReadConfig(const std::string& fname, std::map<string, int>& config)
{
	ifstream ifs(fname);
	if ( !ifs )
	{
...
bool GetConfigEntry(const map<string, int>& config, string& key, int& val)
{
...
bool GetConfigEntry(const map<string, int>& config, const char* key, int& val)
{
	string s(key);
