...
int GetSBuvWC::AddImg(const cv::Mat& Input)
{
    ...
    cv::Mat maskh,maskf,aChannels[3],blue,green,red,redb(_imgRows,_imgCols,CV_8UC1);redb= 0;
    ...
    if(0 && !_weldOff) cv::threshold(aChannels[2],redb,29,255,0);
    else {
        //cout << "_weldOff\n";
...
//        double dl,dr;
    ...
    if(!GetHoughLine(redb(roir),rcut0,rcut1,-1,1.25,1))
    {
...
//        double dl,dr;
    ...
    else
    {
        cv::Mat ctem;
        redb.copyTo(ctem);
        rcut0+= cv::Point2d(roir.x,roir.y);
        cv::Point2d rcut2= rcut0;
        if(rcut0.x < roil.width+roil.x) roil.width= rcut0.x-roil.x;

        if(!GetHoughLine(redb(roil),rcut0,rcut1,-1,0.5))
        {
            _imgNum++;
            return -11;
        }
//        cv::circle(ctem,rcut2,15,cv::Scalar(255));
//        cv::imwrite(_logpd+to_string(_imgNum)+"_"+to_string((dl))+"ctem.jpg",ctem);
        dl= adline[0]*(rcut2.y)-adline[1]*(rcut2.x)-2.75;
        ...
        rdline[2]+= roil.x+mroi.x;
        rdline[3]+= roil.y+mroi.y;
        dr= rdline[0]*(rdline[3]+0)-rdline[1]*(rdline[2]+0);
        //dl= adline[0]*(adline[3]+0)-adline[1]*(adline[2]+0);
    }
    cv::Point2d cross;
...
bool GetSBuvWC::GetHoughLine(const cv::Mat &bimg,cv::Point2d &slcut0,cv::Point2d &slcut1,int SIGN,double disc,double ratio0,double ratio1,double k0,double k1)
{
    ...
    if(disc > 1)
    {
        cv::Mat kernel= cv::Mat(3,3,CV_8UC1,elementArray_33);//Mat::ones(5,7,CV_8UC1);//
        dilate(bimg,bimg,kernel);
    }
    cv::HoughLinesP(bimg,lines,1,CV_PI/180,_houghThresh*disc,_houghMinLineLength*disc,_houghMaxLineGap*ts);
    _log << "slines: \n";
